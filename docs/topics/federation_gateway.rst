...
Implementation
^^^^^^^^^^^^^^
Our `quick start UI <https://github.com/dbosoft/IdentityServer.Quickstart.UI>`_ utilizes some of the below features. Also check out the :ref:`external authentication quickstart <refExternalAuthenticationQuickstart>` and the 
