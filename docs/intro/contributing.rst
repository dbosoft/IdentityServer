...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Please start a discussion on the `core repo issue tracker <https://github.com/dbosoft/IdentityServer/issues>`_.

Bugs and feature requests?
^^^^^^^^^^^^^^^^^^^^^^^^^^
Please log a new issue in the appropriate GitHub repo:

* `Core <https://github.com/dbosoft/IdentityServer>`_
* `AccessTokenValidation <https://github.com/dbosoft/IdentityServer.AccessTokenValidation>`_
